...
pde_t *copyuvm(pde_t *pgdir, uint sz)
{
    ...
    // copy only present pages; leave holes for lazily-allocated regions
    for (i = 0; i < sz; i += PTE_SZ)
    {
...
// On-demand paging: allocate and map a page at fault_addr
int handle_page_fault(uint fault_addr)
{
    if (!proc || !proc->pgdir)
        return -1;
    uint va = align_dn(fault_addr, PTE_SZ);
    // Only handle faults within current process size and user space
    if (va >= proc->sz || va >= UADDR_SZ)
        return -1;
    ...
    if (!mem)
        return -1;
    memset(mem, 0, PTE_SZ);
    *pte = v2p(mem) | (AP_KU << 4) | PE_CACHE | PE_BUF | PTE_TYPE;
    flush_tlb();
    return 0;
}
